...
pub fn find_by_id(email_or_username: &String, pool: &web::Data<Pool>) -> Result<UserDTO, ServiceError> {
    match User::find_user_by_email_or_username(email_or_username, &pool.get().unwrap()) {
...
pub fn signup(user: UserDTO, pool: &web::Data<Pool>) -> Result<String, ServiceError> {
    match User::signup(user.into(), &pool.get().unwrap()) {
    ...
    if let Ok(authen_str) = authen_header.to_str() {
        if authen_str.starts_with("bearer") {
            ...
            if let Ok(token_data) = token_utils::decode_token(token.to_string()) {
                if let Ok(email) = token_utils::verify_token(&token_data, pool) {
                    if let Ok(user) = User::find_user_by_email(&email, &pool.get().unwrap()) {
                        User::logout(&user.email, &pool.get().unwrap());
                        return Ok(());
                    }
                }
            }
